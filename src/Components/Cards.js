...
function Cards(props) {
    ...
    async function query(API_TOKEN, offset, len_of_resp) {
        try {
            const response = await fetch(
                `https://datasets-server.huggingface.co/rows?dataset=deepmind%2Fcode_contests&config=deepmind--code_contests&split=train&offset=${offset}&length=${len_of_resp}`,
                {
    ...
    useEffect(() => {
        async function fetchData() {
            await query(API_TOKEN, offset, len_of_resp);
        }
        fetchData();
    }, [len_of_resp, offset]);
    ...
    useEffect(()=>{
        ...
        switch(selectedDifficulty){
            case "Easy":
                setLowerLimit(0);
                setUpperLimit(6);
                //bhai trial hai, below 4 lines ko erase kar dena agar error aata hai toh
                probs=[];
                // setProbs([]);
                setActualProbs([]);
                setOffset(0);
            ...
            if (dataset && dataset.rows && dataset.rows.length > 0 ) {
                ...
                if(actualProbs.length>1000)
                {
                    setActualProbs((prevActualProbs) => {
                        ...
                        return newActualProbs.slice(0, 1000); // Ensure it contains at most 300 items
                    });
                    // setVodka(!vodka);
